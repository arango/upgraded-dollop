package com.kubetown.sockettest.ioclient.parser;

import com.google.gson.Gson;
import com.google.gson.JsonParseException;
import com.google.gson.JsonSyntaxException;
import com.kubetown.sockettest.APIResult;
    ...
    public static class Encoder {
        ...
        private String encodeAsString(Packet obj) {
            ...
            if (obj.data != null) {
                ...
                Object[] array_values = new Object[arr.length()];
                for (int i = 0; i < arr.length(); i++) {
                    try {
                        array_values[i] = arr.get(i);
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                }

                str.append(gson.toJson(array_values));
//                str.append(obj.data);
            }
    ...
    public static class Decoder extends Emitter {
        ...
        private static Packet decodeString(String str) {
            ...
            if (length > i + 1){
                try {
                    str.charAt(++i);
                    String clean = str.substring(i);
                    JSONArray arr = new JSONArray();
                    try {
                        APIResult[] results = gson.fromJson(clean, APIResult[].class);
                        arr.put(results[0]);
                    } catch (JsonSyntaxException e) {
                        String messageName = clean.substring(0, clean.indexOf(","));
                        messageName = messageName.substring(2);
                        messageName = messageName.substring(0, messageName.length() - 1);
                        clean = clean.substring(clean.indexOf(",") + 1);
                        clean = clean.substring(0, clean.length() - 1);
                        APIResult res = gson.fromJson(clean, APIResult.class);
                        arr.put(messageName);
                        arr.put(res);
                    }
...
// need to mangle this so it's giving us a json array of garbage
